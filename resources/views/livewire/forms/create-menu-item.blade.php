<div>
                ...
                <div
                                            ...
                                            <div>
                                                ...
                                                <p class="text-sm text-gray-500 dark:text-gray-400">
                                                    ...
                                                    @if (restaurant()->currency->currency_position == 'left' || restaurant()->currency->currency_position == 'left_with_space')
                                                        {{ restaurant()->currency->currency_symbol }}{{ $variationPrice[$key] ?? '0.00' }}
                                                    @else
                                                        {{ $variationPrice[$key] ?? '0.00' }}{{ restaurant()->currency->currency_symbol }}
                                                    @endif
                                                </p>
                                            </div>
                                        </div>
                                ...
                                <div>
                                    ...
                                    <div class="space-y-2">
                                        ...
                                        <div class="flex items-center justify-between gap-2 p-3 bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-600">
                                            ...
                                            <div class="relative flex-shrink-0">
                                                <x-currency-input
                                                        wire:model.blur="variationBaseDeliveryPrice.{{ $key }}"
                                                        class="mt-1 pr-10 w-64"
                                                        step="5"
                                                        placeholder="1 000" min="0" />
                                            </div>
                                        </div>
                    ...
                    <div class="space-y-6 mt-4">
                        ...
                        <div>
                            ...
                            <x-currency-input
                                id="itemPrice"
                                wire:model.live.debounce.500ms="itemPrice"
                                class="mt-1 pr-10 w-64"
                                step="5"
                                placeholder="1 000"
                            />
                                    ...
                                    <div class="flex items-center space-x-3 flex-1 min-w-0">
                                        ...
                                        <div class="min-w-0">
                                            <span class="font-medium text-gray-900 dark:text-white text-sm truncate block">
                                                @lang('modules.menu.orderType_delivery')
                                            </span>
                                            <div class="text-xs sm:text-sm text-gray-500 dark:text-gray-400">
                                                @lang('modules.menu.defaultDeliveryPrice')
                                            </div>
                                        </div>
                                    </div>
                                    <x-currency-input 
    ...
    @script
    <script>
    // Auto-propagate base price to Order Types and Delivery when they are empty
    const setupPricePropagation = () => {
        const basePriceInput = document.getElementById('itemPrice');
        ...
        if (basePriceInput) {
            basePriceInput.addEventListener('input', function() {
                ...
                if (basePrice && parseFloat(basePrice) > 0) {
                    // Propager vers les champs Order Types (DineIn, Pickup)
                    @foreach($orderTypes->reject(fn($type) => strtolower($type->slug ?? $type->name) === 'delivery') as $orderType)
                        setTimeout(() => {
                            const input = document.querySelector('[wire\\:model\\.live="orderTypePrices.{{ $orderType->id }}"]');
                    ...
                    // Propager vers le champ Base Delivery Price
                    setTimeout(() => {
                        const deliveryInput = document.querySelector('[wire\\:model\\.live="baseDeliveryPrice"]');
                        if (deliveryInput && !deliveryInput.value) {
                            deliveryInput.value = basePrice;
    ...
    // Execute immediately and after Livewire loads
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', setupPricePropagation);
    } else {
        setupPricePropagation();
    }
    </script>
    @endscript
</div>
